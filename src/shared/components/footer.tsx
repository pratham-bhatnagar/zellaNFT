import React from "react";

export default function Footer() {
  return (
    <div className="flex flex-wrap gap-y-24 p-20 bg-gradient-to-tr from-[#61ABF5] via-[#FC0090] to-[#FE4346] text-white">
      <div className=" flex flex-col items-center gap-6 w-[40%]">
        <p className="flex flex-wrap italic text-4xl justify-center font-bold px-[10%]">
          Let’s begin your NFT journey with ZelaaNFT
        </p>
        <div className="flex w-full m-auto px-[10%] gap-4">
          <button className="py-3 px-8 w-full bg-white text-[#FC0090] font-medium text-lg rounded-xl">
            Sign Up
          </button>
          <button className="py-3 px-8 bg-inherit w-full text-white border-[1px] border-white font-medium text-lg rounded-xl">
            Sign Up
          </button>
        </div>
        <span className="flex gap-2 items-center">
          <img
            className="h-10"
            src="data:image/png;base64,"
          ></img>
          <p className="text-md">Connect to Metamask</p>
        </span>
      </div>
      <div className="w-[20%] flex flex-col gap-5 text-xl items-start pl-10 ">
        <p className="font-semiboldbold text-3xl italic mb-10">ZelaaNFT</p>
        <p>Marketplace</p> <p>Drops</p> <p>Creator</p>
      </div>
      <div className="w-[20%] flex flex-col gap-5 text-xl items-start pl-10 ">
        <p className="font-semiboldbold text-3xl italic mb-10">Community</p>
        <p>About</p> <p>Blog</p> <p>Subscribe</p>
      </div>{" "}
      <div className="w-[20%] flex flex-col gap-5 text-xl items-start pl-10 ">
        <p className="font-semiboldbold text-3xl italic mb-10">Help</p>
        <p>Get in Touch</p> <p>Help Center</p> <p>Bug Reports</p>
      </div>
      <div className="w-[40%]">
        <img
          src="https://zelaanft.whitespacestd.net/wp-content/uploads/2022/07/logo.png"
          className="h-20"
        />
      </div>
      <div className="w-[30%]  pl-10 text-lg">
        <span className="flex gap-5 items-center mb-5">
          <img className="h-6" src="./svgs/facebook-f-brands-white.svg"></img>{" "}
          <img className="h-6" src="./svgs/twitter-brands-white.svg"></img>{" "}
          <img className="h-6" src="./svgs/linkedin.svg"></img>
        </span>
        <p>License No. 90926312</p>
      </div>
      <div className="w-[30%] flex flex-col gap-5 text-lg">
        <p>© 2021 ZelaaNFT. All rights reserved</p>
        <p>Privacy Policy Terms of Service</p>
      </div>
    </div>
  );
}
